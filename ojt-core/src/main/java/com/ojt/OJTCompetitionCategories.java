...
public class OJTCompetitionCategories {
	...
	public static void addNewCompetitionCategory(final CompetitionCategory competitionCategory) {
		...
		OJTConfiguration.getInstance().updateProperty(OJTConfiguration.COMPETITION_CATEGORIES, categoriesAsString());
	}
	...
	private static List<CompetitionCategory> loadCategoriesFromConfiguration() {
		...
		final String[] categoriesAsString = OJTConfiguration.getInstance().getProperty(OJTConfiguration.COMPETITION_CATEGORIES).split(";");
		...
		for(final String categoryAsString : categoriesAsString) {
			if(!categoryAsString.trim().isEmpty()) {
				...
				final CompetitionCategory category = new CompetitionCategory(categParams[0], CompetitorSex.valueOf(categParams[1]));
	...
	public static CompetitionCategory retrieveCategoryFromHumanName(
			String humanName) {
		for(final CompetitionCategory categ : getCategories()) {
		...
		return CompetitionCategory.UNKNOWN_CATEGORY;
	}
