...
public class ExportPanel extends JPanel implements GroupExportListener {
	...
	public void exportFinished() {
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				title.setText("Poules exportées dans le fichier :   ");
	...
	public void process(final File importFile, final List<CompetitorGroup> list,
			final CompetitionDescriptor compDesc, final File modelsFile, final GroupExportListener listener) {
		initProcessing();
		...
		try {
			if (OJTConfiguration.getInstance().getPropertyAsBoolean(OJTConfiguration.EXPORT_FROM_MODELS)
				...
				JOptionPane.showMessageDialog(SwingUtilities.windowForComponent(this),
						"Le fichier de modèle n'existe pas, on utilise le modèle par défaut",
						"Utilisation de modèle", JOptionPane.WARNING_MESSAGE);

			}
			final CompetitorsGroupExporter exporter = CompetitorsGroupExporterFactory.getInstance().createCompetitorsGroupExporter(
					exportFile, modelsFile);
	...
	private void displayExportError(final Exception ex) {
		filenameField.setText("Erreur lors de l'export : " + ex.getCause() == null ? ex.getMessage()
				: ex.getCause().getMessage());
		JOptionPane.showMessageDialog(this,
				"Erreur lors de l'export : " + ex.getCause() == null ? ex.getMessage()
						: ex.getCause().getMessage(), "Erreur d'export", JOptionPane.ERROR_MESSAGE);
	}

	private void buildGui() {
		setLayout(new GridBagLayout());
		title = new JLabel("Export en cours :                     ");
		add(title, new GridBagConstraints(0, 0, 1, 2, 0.5, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
		filenameField = new JTextField();
		filenameField.setEditable(false);
		filenameField.setBorder(null);
		filenameField.setText("Lancement de l'export en cours...");
		add(filenameField, new GridBagConstraints(1, 0, 1, 1, 2.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
		progressBar = new JProgressBar();
		add(progressBar, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0));
		...
		if (System.getProperty("os.name").contains("Windows")) {
			add(openButton, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
