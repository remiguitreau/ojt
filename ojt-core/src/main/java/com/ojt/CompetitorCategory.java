...
public enum CompetitorCategory {
	...
	JUNIOR("Junior") {
		@Override
		public boolean isCompetitorFromCategory(final int birthYear, final int yearOfReference) {
			return (yearOfReference - birthYear) == 17 || (yearOfReference - birthYear) == 18 || (yearOfReference - birthYear) == 19;
		}
	},
	SENIOR("Senior") {
		@Override
		public boolean isCompetitorFromCategory(final int birthYear, final int yearOfReference) {
			return (yearOfReference - birthYear) >= 20;
		}
	};
	...
	public static CompetitorCategory retrieveCategoryFromBirthDateAndCompetitionDate(final Date birthDate, final Date competitionDate) {
		...
		for(final CompetitorCategory competitorCategory : CompetitorCategory.values()) {
			if(competitorCategory != UNKNOWN && competitorCategory.isCompetitorFromCategory(birthYear, referenceYear)) {
	...
	public static CompetitorCategory retrieveCategoryFromHumanName(
			String humanName) {
		for(final CompetitorCategory competitorCategory : CompetitorCategory.values()) {
