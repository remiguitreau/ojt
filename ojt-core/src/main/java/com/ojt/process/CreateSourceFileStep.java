...
/**
 * Etape de création du fichier source
...
public class CreateSourceFileStep extends AbstractStep {
	...
	public void process(final CompetitionDatas competitionDatas) {
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				final File sourceFile = new File(OjtConstants.SOURCE_DIRECTORY,
						FileNameComposer.composeFileName(competitionDatas.getCompetitionDescriptor(),
				...
				try {
					...
					exportLabel.setText("Fichier source enregistré dans le fichier : "
							+ sourceFile.getAbsolutePath());
				} catch (final Exception ex) {
					logger.error("Erreur lors de la création du fichier source : " + sourceFile, ex);
	...
	public String getTitle() {
		return "Création du fichier source";
	}
