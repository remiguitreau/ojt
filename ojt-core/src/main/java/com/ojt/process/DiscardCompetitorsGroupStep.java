...
public class DiscardCompetitorsGroupStep extends AbstractStep {
	...
	public String getTitle() {
		return "Gestion des groupes de compétiteurs écartés du tri";
	}
	...
	public void process(final CompetitionDatas competitionDatas) {
		try {
			if (isAtLeastOneSavedGroup(competitionDatas.getCompetitorsGroups())) {
				...
				final CompetitorsDao competitorsDao = CompetitorsDaoFactory.createCompetitorsDao(savedFile,
						true);
				...
				for (; competitorsGroupIterator.hasNext();) {
					...
					logger.info("Groupe '" + competitorGroup.getName() + "' réservé : "
							+ competitorGroup.isSavedForAnotherWeighing());
			...
			JOptionPane.showMessageDialog(null, "Le fichier de réserve n'a pas pu être créé.", "OJT",
					JOptionPane.ERROR_MESSAGE);
			logger.error("Erreur lors de la création du fichier de réserve.", ex);
		}
		stepFinish();
	}
	...
	private File copyCompetitionFileToSavedFile(final CompetitionDatas competitionDatas) throws IOException {
		final File savedFile = new File(OjtConstants.WEIGHING_DIRECTORY,
				FileNameComposer.composeSavedFileForAnotherSortName(
