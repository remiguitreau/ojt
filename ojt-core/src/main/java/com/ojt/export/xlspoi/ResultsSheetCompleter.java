...
public class ResultsSheetCompleter implements GroupExportListener {
	...
	public void appendGroup(final CompetitorGroup group, final int groupNumber, final boolean isFirstGroup) {
		...
		if (!isFirstGroup) {
			...
			currentGroupCell.setCellValue(new HSSFRichTextString(completeWithGroupInfos(
					XlsPoiUtilities.getCellValueAsString(groupCell), group, groupNumber)));
			currentRow++;
		}
		for (int i = (isFirstGroup ? 1 : 0); i < group.getCompetitors().size(); i++) {
			...
			for (int col = 0; col < 3; col++) {
				...
				if ((cellText != null) && !cellText.trim().isEmpty()) {
					newCell.setCellValue(new HSSFRichTextString(completeWithCompetitorInfos(cellText,
							group.getCompetitors().get(i))));
				}
			}
			currentRow++;
		}
	...
	public void cleanSheet() {
		if(groupCell != null) {
			groupCell.setCellValue(new HSSFRichTextString(completeWithGroupInfos(
					XlsPoiUtilities.getCellValueAsString(groupCell), firstGroup, 1)));
			for (int col = 0; col < 3; col++) {
				final HSSFCell cell = resultsSheet.getRow(competitorRowIndex).getCell(col);
				final String cellText = XlsPoiUtilities.getCellValueAsString(cell);
				if ((cellText != null) && !cellText.trim().isEmpty()) {
					cell.setCellValue(new HSSFRichTextString(completeWithCompetitorInfos(cellText,
							firstCompetitor)));
				}
			}
		}
	...
	public void groupExportBegin(final String groupName, final CompetitorGroup group, final int groupNumber) {
		...
		try {
			...
			throw new RuntimeException("Erreur lors de l'ajout du groupe aux résultats", ex);
		}
	}
